...
switch ($_GET["op"]) {
    ...
    case "turnosxvendedor":
        ...
        $sql = "select s.*,fun_nombre_agente(s.agnomb)nombre,s.agnomb nombc03 from(
        SELECT e.agnomb, COUNT(*) cant , IFNULL(SUM(IF(e.`factur`!='',1,0)),0)cantmont 
        ,ROUND(sum(IFNULL(IF(e.factur!='',e.`monto`,0),0))-sum(IFNULL(n.`monto`,0)),2)monto 
        FROM `ticket_enc` e 
        LEFT JOIN notas n ON n.factura=e.factur AND n.fecha BETWEEN '$fini' AND '$ffin 23:59:59' 
        where $wher AND e.destin='v' 
        GROUP BY agnomb) as s ORDER BY monto DESC, cant DESC;";
    ...
    case "emisionesporhora":
        ...
        //     SELECT tienda.NombreHora(DATE_FORMAT(fechac,'%H'))hora, COUNT(*)cant FROM `ticket_enc` e where $wher GROUP BY DATE_FORMAT(fechac,'%H')) AS s;";
            ...
            FROM ( 
            SELECT NombreHora(DATE_FORMAT(fechac,'%H'))hora, SUM(IF(e.`destin`='V',1,0))vent,SUM(IF(e.`destin`='C',1,0))caja, SUM(IF(e.`destin`='E',1,0))entr FROM `ticket_enc` e 
