<style>
    /* ========================================================================
       Estilos Visuales para Tarjetas de Trazabilidad e Informes
       ======================================================================== */
    .mialert {
...
/**
 * ============================================================================
 * Sistema de Control de Filas - Controlador AJAX de Informes y Reportes (a_informes.php)
 * ============================================================================
 * Este controlador procesa todas las consultas analíticas, estadísticas e informes:
 * 1. 'turnosresumido'          : Resumen de turnos emitidos y completados por sede.
 * 2. 'traza_x_turno'           : Trazabilidad cronológica detallada de un turno (etapas, tiempos, montos).
 * 3. 'traza_turnos'            : Selector de turnos disponibles para trazabilidad.
 * 4. 'comp_dia_emisi_x_hora_d' : Comparativa de visitas/emisiones por hora entre dos fechas con gráficos (Gauge y Líneas).
 * 5. 'turnos_por_ubicacion'    : Análisis de turnos y tiempos promedio por módulo (Ventas, Cajas, Entregas).
 * 6. 'estado_en_linea'         : Monitor en tiempo real por sucursal (total, completados, abandonos, tiempos).
 * 7. 'estudio_detallado'       : Matriz de flujo, tasas de derivación, embudo de abandono y completación global.
 * ============================================================================
 */
...
// Inclusión de la capa de conexión y funciones auxiliares de tipos
require "../config/Conexion.php";
require "a_tools.php";

// Cláusula WHERE dinámica enviada por el formulario de filtros y búsqueda
...
switch ($_GET["op"]) {
     ...
     * Caso: turnosresumido
     * ------------------------------------------------------------------------
     * Consulta el total de turnos emitidos y completados por sede,
     * generando dinámicamente la tabla HTML con sumatorias en el pie de página.
     */
    case 'turnosresumido':
        ...
        <table id="tdatos" class="table table-sm table-striped table-bordered table-condensed table-hover">
            <thead>
                <tr>
                <?php
                    // Mapeo automático de nombres y tipos de columnas
     ...
     * Caso: traza_x_turno
     * ------------------------------------------------------------------------
     * Muestra la línea de tiempo (trazabilidad) de un tiquete específico,
     * detallando cada estación por la que transitó, fecha/hora y duración.
     */
    case "traza_x_turno":
        // Unión de la creación del encabezado con el historial de eventos en ticket_det
        ...
        // Distintivo visual del tipo de documento (Contado vs Crédito)
        if ($tidoc == 'V') { ?>
        ...
        // Renderizado de tarjetas de trazabilidad
        echo '<div class="row">';
        while ($reg = $rspta->fetch_array()) {   
            echo '<div class="col-lg-4 col-md-6 col-sm-12">';
            echo '<div class="alert alert-info mialert">';
            
            $nombre = ($reg["subproc"] == 'prest') ? "SOLICITA PRESTAMO" : $reg["nombre"];
            ...
            // Detalles según el tipo de estación/ubicación
            switch ($reg["ubicac"]) {
                case '2': // Módulo de atención / ventas
     ...
     * Caso: traza_turnos
     * ------------------------------------------------------------------------
     * Construye un elemento <select> con los tiquetes encontrados para filtrar su trazabilidad.
     */
    case "traza_turnos":
    ...
    /**
     * ------------------------------------------------------------------------
     * Caso: comp_dia_emisi_x_hora_d
     * ------------------------------------------------------------------------
     * Reporte comparativo de afluencia por hora entre dos fechas seleccionadas,
     * integrando medidores tipo velocímetro (Gauge) y un gráfico de líneas continuo.
     */
    case "comp_dia_emisi_x_hora_d":
        ...
        $sql = "SELECT NombreHora(DATE_FORMAT(fechac,'%H')) hora, IFNULL(f1.cant,0) c1, IFNULL(f2.cant,0) c2 FROM `ticket_enc` e 
        LEFT JOIN (SELECT NombreHora(DATE_FORMAT(fechac,'%H')) hora, COUNT(*) cant FROM `ticket_enc` e WHERE $wher AND DATE_FORMAT(e.fechac,'%Y-%m-%d') = '$fecha1' GROUP BY DATE_FORMAT(fechac,'%H')) AS f1 ON f1.hora=NombreHora(DATE_FORMAT(fechac,'%H'))
        LEFT JOIN (SELECT NombreHora(DATE_FORMAT(fechac,'%H')) hora, COUNT(*) cant FROM `ticket_enc` e WHERE $wher AND DATE_FORMAT(e.fechac,'%Y-%m-%d') = '$fecha2' GROUP BY DATE_FORMAT(fechac,'%H')) AS f2 ON f2.hora=NombreHora(DATE_FORMAT(fechac,'%H'))
        ...
        // Cálculo dinámico de escalas para los medidores Gauge
        $max = $tot1;
        ...
        echo '</div>';
        ?>           
        <!-- Script para dibujar los medidores Google Gauge -->
        ...
        <!-- Script para dibujar el gráfico de líneas comparativo -->
        <script type="text/javascript">
    ...
    /**
     * ------------------------------------------------------------------------
     * Caso: turnos_por_ubicacion_enc
     * ------------------------------------------------------------------------
     * Contenedor y formulario disparador para el reporte de turnos por ubicación.
     */
    ...
    /**
     * ------------------------------------------------------------------------
     * Caso: turnos_por_ubicacion
     * ------------------------------------------------------------------------
     * Calcula los turnos en espera y atención para Ventas, Cajas y Entregas,
     * junto con los tiempos promedios convertidos a formato legible.
     */
    case "turnos_por_ubicacion":
        ...
        while ($reg = $rspta->fetch_array()) {  
            ...
            <?php
            $tot1 += $reg["EspV"]; $tot2 += $reg["AteV"]; $tot3 += $reg["EspC"];
            $tot4 += $reg["AteC"]; $tot5 += $reg["EspE"]; $tot6 += $reg["AteE"];
            $ttot1 += $reg["tEspV"]; $ttot2 += $reg["tAteV"]; $ttot3 += $reg["tEspC"];
            $ttot4 += $reg["tAteC"]; $ttot5 += $reg["tEspE"]; $ttot6 += $reg["tAteE"];
        }
        ...
        echo '</div></div>';
        ?>
        <!-- Inicialización de DataTables con botones de exportación (Excel, PDF, Imprimir) -->
        <script type="text/javascript"> 
    ...
    /**
     * ------------------------------------------------------------------------
     * Caso: estado_en_linea
     * ------------------------------------------------------------------------
     * Monitor en tiempo real de sucursales con indicadores de turnos completos,
     * abortados y promedios de tiempo de espera, atención y respuesta global.
     */
    ...
    /**
     * ------------------------------------------------------------------------
     * Caso: estudio_detallado
     * ------------------------------------------------------------------------
     * Matriz avanzada de análisis de embudo:
     * - Flujo de clientes entre Ventas -> Cajas -> Entregas.
     * - Abandono discriminado por estación y transiciones.
     * - Tasas de completación parcial y Tasa de Completación Global.
     * - Tiempos promedio de espera y atención por área.
     */
    case "estudio_detallado":
        $g20 = 0; $h21 = 0; $i22 = 0; $h18 = 0; $i17 = 0; $j16 = 0;
        $j20 = 0; $j21 = 0; $j22 = 0; $i20 = 0;
        
        // Conteo por departamento inicial y completados
        ...
        // Ventas derivadas directamente a entregas
        ...
        // Fórmulas matemáticas de balance de derivación y abandono
        $j17 = $h21 - $i17 - $j21;
        $i18 = $g20 - $h18 - $i20;
        $j18 = $i20 - $j20;
        ...
        $h23 = $h20 + $h21; // Total en cajas
        $i23 = $i20 + $i21 + $i22; // Total en entregas
        $j23 = $j20 + $j21 + $j22; // Total de turnos completados
        $k23 = $g20 + $h21 + $i22; // Total de visitas

        // Consulta de tiempos promedio por área (Espera y Atención)
        ...
        while ($reg = $rspta->fetch_array()) { 
            switch ($reg['area']) {
                case 'EV': $ev = $reg['tiempo']; break;
                case 'AV': $av = $reg['tiempo']; break;
                case 'EC': $ec = $reg['tiempo']; break;
                case 'AC': $ac = $reg['tiempo']; break;
                case 'EE': $ee = $reg['tiempo']; break;
                case 'AE': $ae = $reg['tiempo']; break;                            
            }
        }
        ...
        <div class="table-responsive">
            ...
            <tr>    
                ...
                <?php 
                    echo ($j20 == 0) ? "0" : number_format($j20 / $g20 * 100, 0);
                ?>
                %</label></td>
            </tr>
            <tr>
                ...
                <?php 
                    echo ($j21 == 0) ? "0" : number_format($j21 / $h21 * 100, 0);
                ?>
                %</label></td>
            </tr>
            <tr>
                ...
                <?php 
                    echo ($j22 == 0) ? "0" : number_format($j22 / $i22 * 100, 0);
                ?>
                %</label></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <?php
                echo ($j23 == 0) ? "0" : number_format($j23 / $k23 * 100, 0);
                ?>
                %</label></td>                   
            </tr>
            </table>
...
/**
 * Convierte un valor numérico de segundos a una cadena formateada en Horas, Minutos o Segundos
 * 
 * @param float|int $segundos - Cantidad de segundos transcurridos
 * @return string Tiempo formateado (ej: "12 Seg", "04:30 Min", "1:15:00 Horas")
 */
