...
public function mostrar($agenci, $ubmenu, $codigo){
	$sql="SELECT * FROM opcmenu WHERE WHERE agenci=(SELECT n_sede FROM mhosts WHERE dir_ip='".$_SESSION["ip"]."') and ubmenu='$ubmenu' and codigo='$codigo'";
...
public function opmenu_enc($nombre){
	//comentado el 07/02/2025, para obtener la letra incial del comprobante
	//$sql="SELECT * FROM opmenu_enc WHERE agenci in ('00',(SELECT n_sede FROM mhosts WHERE dir_ip='".$_SESSION["ip"]."')) and nombre='$nombre' and estado=1";
	...
	WHERE e.agenci IN ('00',(SELECT n_sede FROM mhosts WHERE dir_ip='".$_SESSION["ip"]."')) AND e.nombre='$nombre' AND e.estado=1;";
	//echo $sql;
	return ejecutarConsultaSimpleFila($sql);
}

public function opmenu_det($ubmenu){
	$sql="SELECT * FROM opmenu_det WHERE agenci in ('00',(SELECT n_sede FROM mhosts WHERE dir_ip='".$_SESSION["ip"]."')) and ubmenu='$ubmenu' and estado=1 order by ordenm";
	//echo $sql;
	return ejecutarConsulta($sql);
}
...
public function listar_v($agenci, $nombre){
	$sql="SELECT DISTINCT b.`nombc03`, IFNULL(i.`nombl03`,b.`nombl03`)nombl03, IF(IFNULL(i.imagen,'')='','guaca.jpg',i.imagen)imagen, IFNULL(i.`activo`,1)activ, IFNULL(i.gc,0)gc, b.local FROM 
	`cias_vendedores` b
	inner JOIN `cias_vendedores_img` i ON i.`nombc03`=b.`nombc03` AND i.`fechlog` > DATE_FORMAT(NOW(),'%Y-%m-%d')
	WHERE i.sucur03=(SELECT n_sede FROM mhosts WHERE dir_ip='".$_SESSION["ip"]."') AND i.logue03=1";
	$sql .= " HAVING activ=1";
